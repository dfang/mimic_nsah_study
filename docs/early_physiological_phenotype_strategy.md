...
- 项目中间表与最终表位置：
  - `mimic-study-498508.asah_study`

建议所有中间表使用 `CREATE OR REPLACE TABLE mimic-study-498508.asah_study.<table_name>`。
...
- 表型与住院死亡：
  - Logistic regression。
  - 参考组：死亡率最低或“轻度生理紊乱型”。
  - 调整变量：年龄、性别、种族、入院类型、Glasgow Coma Scale (GCS)、WFNS（若可得或以 `wfns_gcs_grade_min_48h` 近似）、SAPS III（若有经验证来源）、SOFA，必要时加保险或合并症。
...
- Model B：GCS、GCS grade、GCS motor、WFNS、SAPS III、SOFA 的单变量或小模型比较。
- Model C：8 个生理指标。
- Model D：phenotype。
