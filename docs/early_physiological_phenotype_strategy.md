...
- 保留首次 ICU stay。
- ICU stay 时长 `>= 24` 小时。
- ICU 入科后 0-48 小时 8 个核心聚类变量最多缺失 2 个。
...
同时必须保留一个高特异性动脉瘤证据亚组，定义为 `nsah_evidence_level >= 2 OR has_aneurysm_dx = 1 OR has_aneurysm_procedure = 1`。该亚组不替代主队列，而用于审稿敏感性分析：在更接近经典 aSAH 的患者中，重复主 `log1p(creatinine/INR) + Z-score + PCA(3PC) + K-means K=3` 流程，并检查表型中心、住院死亡梯度、SOFA/SAPS II/OASIS/LODS 外部严重程度验证和调整后死亡关联是否与主分析方向一致。当前脚本输出 `phenotype_strict_aneurysm_subgroup_*` 系列表。
...
K-means 对缺失和插补敏感。如果缺失过多，聚类结果会反映“数据缺失模式”而不是生理表型。8 个核心变量最多缺失 2 个，可以在保留样本量的同时降低插补主导聚类的风险；total GCS 和 GCS grade 不计入主聚类完整度，仅用于描述和敏感性比较。
...
| Level 2 | Level 1 + 破裂动脉瘤性 SAH 证据：ICD-10 `I60.0-I60.6`、ICD-9 `430` 或 clip/coil/embolization 处置证据；`437.3/I67.1` 仅代表未破裂动脉瘤，单独报告为 `has_unruptured_aneurysm_dx` | 协变量、分层和更高特异性敏感性分析     |
...
MIMIC-IV BigQuery 的 ICD 代码通常不含小数点，因此 SQL 中必须使用 `REPLACE(UPPER(icd_code), '.', '')` 后匹配。`4373/I671%` 应只用于未破裂动脉瘤审计，不应命名为破裂 aSAH 诊断率；主表应优先报告 `has_ruptured_aneurysmal_sah_evidence` 和 `has_aneurysm_procedure`。
...
- `mimiciv_3_1_derived.chemistry`：creatinine、sodium。
- `hosp.labevents` + `hosp.d_labitems`：INR 动态识别，作为主聚类凝血功能变量；sodium 保留为描述/敏感性字段。
...
乳酸、肌钙蛋白和氧合比值的缺失不是同一类问题：

- `troponin_peak_48h`：肌钙蛋白是按临床怀疑心肌损伤选择性检测，0-48h 缺失高主要反映未测量，而不是提取失败。脚本额外保留 `troponin_peak_m6_48h` 作为 ICU 前 6 小时至 ICU 后 48 小时敏感性变量，但仍不应进入主聚类。
- `lactate_max_48h`：乳酸也存在选择性检测。为纳入急诊/入 ICU 前已完成的早期乳酸，脚本额外保留 `lactate_max_m6_48h`，仅用于描述和敏感性分析。
- ICU 前 6 小时窗口可以作为“早期入 ICU 周边生理状态”的预先定义敏感性/描述窗口，尤其适合乳酸这类常在急诊或 ICU 转入前完成的指标；但主聚类仍以 ICU 入室时间为 T0，使用 0-48h 核心变量，避免把院前/急诊阶段与 ICU 早期阶段混成同一个主暴露定义。
- `pao2_fio2_min_48h`、`spo2_fio2_min_48h` 和 `oxygenation_min_48h`：这些变量受血气和 FiO2 同时测量限制。对于没有血气或没有 FiO2 记录的患者，不应强行假设 FiO2=0.21；主聚类继续使用低缺失的 `spo2_min_48h`。

### 4.1.4 0-48h 聚合规则要按变量生理意义决定

不要所有变量都取平均值。主聚类应使用能代表“最差早期状态”的指标：

- Hb：最低值，用作主聚类贫血负担变量；输血前最低 Hb 用于敏感性分析。
- GCS motor：最低 GCS motor 用作主聚类神经功能变量；total GCS 和 GCS grade 同步报告并作为替代敏感性变量。若 Phenotype 2 和 Phenotype 3 的 GCS motor 中位数同为 1，主文不应把二者差异解释为更重神经功能损伤，而应说明 P3 相对 P2 的额外风险主要来自肾功能、凝血、血小板和氧合异常。脚本输出 `phenotype_gcs_motor_distribution`，用于展示 GCS motor 的完整分布，而不是只依赖中位数。
- MAP：最低值。
- Shock index：最大值。
...
- Creatinine：最大值。
- INR：最大值，用作主聚类凝血功能变量。
- Sodium：最大值，保留为描述和敏感性变量，不进入本轮主聚类。
- Platelet：最低值。

可同时保留 `mean`, `first`, `last`, `measurement_count`，但主聚类只用预先定义的 8 个极值，避免分析者自由度过大。

### 4.1.4.1 24h window 敏感性分析

为回应“0-48h 是否过长、是否受到 ICU 后治疗过程影响”的质疑，脚本额外生成同源 0-24h 核心变量：
...
该敏感性分析复用主流程：对 `creatinine_max_24h` 和 `inr_max_24h` 做 `log1p`，随后 Z-score、PCA 取 3 个 PC、K-means K=3。结果写入 `phenotype_24h_window_sensitivity` 和 `phenotype_24h_window_centers_zscore`。24h 方案不替代主分析，因为 INR、Hb、肌酐、血小板在更短窗口内缺失率略高；它用于证明主要死亡梯度和表型结构不是 48h 窗口任意选择造成的。

### 4.1.4.2 严重程度评分只作外部验证

SOFA、SAPS II、OASIS 和 LODS 不进入聚类输入。原因是这些评分本身已经综合了多个器官功能和治疗信息，若纳入聚类会让 phenotype 退化成传统严重程度评分的再包装。它们应作为外部验证指标，用于检查 phenotype 是否呈一致的严重程度梯度。脚本输出 `phenotype_external_severity_validation`，报告各 phenotype 的 SOFA/SAPS II/OASIS/LODS 中位数、Kruskal-Wallis p 值和 phenotype 顺序的 Spearman 相关。

此外，脚本输出 `phenotype_severity_score_adjusted_models`，用于直接回应“phenotype 只是重建 SOFA/传统严重程度评分”的审稿质疑。该表包括：

- `phenotype_plus_sofa`
- `phenotype_plus_sapsii`
- `phenotype_plus_oasis`
- `phenotype_plus_lods`
- `phenotype_plus_sofa_clinical`：phenotype + SOFA + 年龄 + 性别 + 入院类型 + 早期贫血 + 动脉瘤证据

这些模型只用于关联强度和增量信息验证，不应写成因果效应。若 phenotype 在加入传统评分后仍与住院死亡显著相关，可表述为“the phenotype solution captured prognostic information not fully explained by established ICU severity scores”。
...
贫血主定义可以使用 `hb_min_48h_all`，但敏感性分析应使用 `hb_min_48h_pre_transfusion` 或排除输血患者，避免“输血后 Hb 被治疗改变”影响贫血分类。
...
建议采用 8 个常规可提取、低缺失指标，对应文档中的 8 个生理维度。本轮主聚类保留最低 Hb 和最低 GCS motor，剔除 sodium，新增最高 INR 作为凝血功能维度；同时彻底剔除高缺失的乳酸、PaO2/FiO2 和合成氧合比值；不构造“终末器官低灌注复合评分”，避免引入未经验证的自创评分工具。
...
| --------- | --------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| 贫血      | `hb_min_48h_all`      | 0-48h 最低 Hb                                                  | 优先 `mimiciv_3_1_derived.complete_blood_count.hemoglobin`；原始审计 `hosp.labevents` itemid `51222` |
...
| 凝血功能  | `inr_max_48h`         | 0-48h 最高 INR                                                 | 通过 `hosp.d_labitems` 动态识别 label 包含 INR 的血液检验项目，并从 `hosp.labevents` 提取             |
...
- `sodium_max_48h`：0-48h 最高钠，保留为描述和敏感性变量；本轮主聚类剔除 sodium，以测试凝血功能维度替代电解质/渗透压维度后的表型稳定性。
...
- `sapsii_24h`、`sofa_24h`：来自 MIMIC-IV 3.1 derived 的 `sapsii` 和 `first_day_sofa`，用于描述、预测性能比较和扩展结局模型协变量，不建议纳入主聚类，避免和核心生理变量重复。MIMIC-derived 当前提供 SAPS II/APS III/OASIS/LODS，而不是 SAPS III。
...
4. 对右偏的 `creatinine_max_48h` 和 `inr_max_48h` 先做 `log1p`，再对 8 个聚类变量做 Z-score 标准化。
5. 提取前三个 PCA 主成分后进行 K-means；同时透明报告实际解释方差。
6. 固定随机种子 `42`。
...
- 算法：`log1p(creatinine_max_48h, inr_max_48h)` + Z-score + PCA + K-means。
...
建议主分析默认目标：`K=3`。当前优化后，log-PCA K=3 比原始 8 维 K-means 具有更好的几何分离和 bootstrap 稳定性，同时保留清晰死亡率梯度。原始 8 维 K-means 不再作为主分析，而作为敏感性参照，用于说明未经变换的特征空间仍能得到方向一致的风险分层。

推荐报告结构：

- 主分型：log-PCA K=3，用于主要表型命名、主要结局比较、贫血交互和回归模型。
- 原始 8 维 K-means K=3：作为敏感性分析，输出 `phenotype_raw_kmeans_*_sensitivity`。
- Model-based LPA/GMM 敏感性：在同一 log-PCA 3PC 空间运行 Gaussian mixture 近似 LPA，比较 K=2-5 的 BIC、AIC、entropy、最小类比例和死亡率梯度，输出 `phenotype_lpa_*_sensitivity`。只有当 BIC 支持 K=3、entropy `>=0.80`、ARI vs 主 log-PCA K-means `>=0.70` 且最小类比例 `>=5%` 时，才考虑把 LPA/GMM 升级为主方案；否则保留为补充敏感性分析。
- 高分辨率敏感性分型：K=4，用于说明真实世界重症人群中存在小型极高危尾部；不在 K=4 小类内做复杂贫血分层回归。
- 交叉验证：报告 K=3 与 K=4 assignment 交叉表，确认 K=4 极高危小亚型主要来自 K=3 的重症组。
- 病因高特异性敏感性：仅保留有动脉瘤诊断或处置证据的患者，重复主 log-PCA K=3 流程，输出 `phenotype_strict_aneurysm_subgroup_assignments`、`phenotype_strict_aneurysm_subgroup_summary`、`phenotype_strict_aneurysm_subgroup_outcomes`、`phenotype_strict_aneurysm_subgroup_severity_validation`、`phenotype_strict_aneurysm_subgroup_regression` 和 `phenotype_strict_aneurysm_subgroup_severity_score_adjusted_models`。

稳定性验证：

- Bootstrap 重采样后计算 cluster assignment 稳定性，主表 `phenotype_bootstrap_stability` 对应 log-PCA K=3；原始 K-means 稳定性另存 `phenotype_raw_kmeans_bootstrap_stability_sensitivity`。重点报告 bootstrap ARI 的均值、中位数、IQR 和低分位数；若稳定性不足，论文中应把表型定位为探索性、假设生成。
- 替代算法：hierarchical clustering，输出 `phenotype_cluster_stability` 中的 K-means vs hierarchical ARI；主分析中该比较在 PCA score 空间计算。
...
- LPA/GMM 敏感性：使用同一 log-PCA 3PC 输入空间，避免把 GCS motor 和 SpO2 等离散/边界变量直接作为正态连续变量强行建模。Python 脚本使用 `GaussianMixture` 作为 LPA 的 model-based clustering 近似，并报告描述性 likelihood-ratio 近似值；正式 LMR-LRT 如需主文使用，应再用 R `tidyLPA/mclust` 或 Mplus 复核。
- 对 24h 窗口重复聚类。
- 排除所有输血患者后重复聚类。
- 严格动脉瘤证据亚组重复聚类，用于说明主结果不是由 non-aneurysmal SAH 混入造成。
...
- 使用 cluster center 的方向解释：例如低 Hb、低 GCS motor、低 MAP、高 shock index、低 SpO2、高 creatinine、高 INR、低 platelet；sodium、total GCS 和 GCS grade 用于描述和敏感性验证。
...
- Table 1 至少包含年龄、性别、合并后的种族分组、入院类型、保险、aneurysm evidence level、动脉瘤诊断/处置证据、早期贫血、输血、ICU/住院时长、total GCS、GCS motor、GCS grade、sodium、ePVS、troponin、SAPS II/SOFA/APS III/OASIS/LODS、过程性治疗/专科干预和 8 个主聚类变量。主表种族建议使用 `race_group`：White/Black/Other/Unknown；原始 MIMIC race 亚类因 cell size 小，放入补充表即可。total GCS 和 GCS grade 可作为描述或补充材料变量，不作为主聚类输入。

过程性治疗和专科干预应作为 Table 1 的扩展行展示，但不进入主聚类：

- 通用生命支持：`vasopressor_48h`、`mech_vent_48h`、`any_rbc_transfusion_48h`、`crrt_48h`。
- SAH/神经专科过程指标：`aneurysm_securing_hosp`、`evd_or_icp_48h`、`nimodipine_48h`。
- 容量管理：`fluid_input_ml_48h`、`fluid_output_ml_48h`、`fluid_balance_l_48h` 和 `fluid_balance_complete_48h`。

这些变量代表医疗过程、治疗选择和疾病严重程度的共同结果，可能位于早期生理异常到死亡之间的中介路径上。因此主文应把它们称为 `process-of-care variables`，用于资源使用描述和扩展敏感性调整，不应写成无偏因果混杂控制。
...
- 扩展模型可加入种族、保险、SAPS II、SOFA、APS III、OASIS、LODS 或 WFNS 近似变量，但要避免把主聚类变量全部再作为协变量纳入同一个模型，否则会过度调整并削弱 phenotype 的整体生理意义。
- 这些模型用于检验 phenotype 与住院死亡的独立关联，不用于证明 phenotype、贫血或 INR 对死亡的因果效应。若纳入机械通气、升压药、SOFA 等早期严重程度变量，应作为扩展/敏感性模型解释，因为这些变量可能位于疾病严重程度到死亡之间的中介路径上。
  ...
  - 当前脚本输出 `phenotype_severity_score_adjusted_models`：检验 phenotype 在 SOFA/SAPS II/OASIS/LODS 或 SOFA+临床协变量调整后是否仍与住院死亡相关，用于说明本研究不是简单复刻传统 ICU 严重程度评分。
  - 当前脚本输出 `phenotype_process_of_care_audit` 和 `phenotype_process_of_care_adjusted_models`：前者用于 Table 1 展示过程性治疗和液体平衡，后者采用递进扩展模型加入动脉瘤处置、尼莫地平、EVD/ICP、血管活性药、机械通气、RBC、CRRT 和液体平衡。该模型只能解释为 process-of-care adjusted sensitivity analysis，因为这些变量可能既是治疗决策，也是疾病严重程度和中介过程的标志。
...
- 生存时间：
  - 当前脚本输出 `phenotype_survival_km_curve`、`phenotype_survival_logrank` 和 `phenotype_survival_cox_models`。
  - 该分析使用 `hospital_los_days` 作为时间，`hospital_mortality` 作为事件；活着出院者按出院时间删失。因此它只能解释住院期死亡时间分布，不能解释出院后的长期生存。
  - Cox 模型用于补充 phenotype 与住院期死亡风险的 time-to-event 关联，仍应使用“associated with”，不要写成因果效应。`cox_phenotype_process_of_care_exploratory` 是固定协变量探索性敏感性模型，不替代主 Cox 模型；若正式调整 0-48h 过程治疗变量，应改用 time-varying covariates 以避免 immortal time bias。
...
- 当前脚本输出 `phenotype_feature_heatmap`，以长表形式同时保留每个 phenotype 的标准化中心和原始 median [IQR]，可直接生成主文热图。
- K=3 各 phenotype 住院死亡率柱状图，带样本量和 95% CI。
- K=3 各 phenotype 住院期 Kaplan-Meier 曲线，作为死亡时间分布的补充展示。
...
SHAP 不直接解释 K-means。当前脚本采用更保守、可复现且不新增依赖的代理任务：

1. 死亡预测解释：
   - 用 8 个主聚类变量拟合标准化 logistic mortality model，其中 creatinine 和 INR 采用 `log1p` 变换。
   - 对线性 logistic 模型，特征对 log-odds 的贡献可近似写作 `beta_j * z_j`；脚本输出 `phenotype_shap_feature_importance` 和 `phenotype_shap_phenotype_contributions`，并明确标注 `linear_logistic_shap_approximation`。
   - 该结果用于解释死亡预测模型中的变量贡献，不应用来宣称某变量“导致”聚类或死亡。

2. 可选增强：
   - 如果后续允许新增依赖，可用 XGBoost/RandomForest 预测 `phenotype` 或 `hospital_mortality`，再使用正式 SHAP 包生成非线性解释图。
   - 这类 black-box 解释应放入补充材料，主文仍以标准化中心、原始中位数热图和回归/生存模型为核心。
...
当前脚本输出 `phenotype_prediction_metrics`。理想结果不是 phenotype-only 超过 8 个原始连续变量；从连续变量压缩为离散 phenotype 必然损失部分预测信息。若 8-feature 模型 AUROC 高于 phenotype-only，应把 phenotype 定位为临床解释和亚组描述工具，而不是最优死亡预测器。可报告 phenotype 相对 GCS-only 有明显改善，但避免把本文主卖点写成预测性能。

## 12. 敏感性分析

必须做：

- 24h 窗口替代 48h 窗口。当前 SQL 已在 48h 宽表中同步生成同源 24h 核心变量，脚本输出 `phenotype_24h_window_sensitivity` 和 `phenotype_24h_window_centers_zscore`。
- 严格动脉瘤证据亚组：仅保留 `nsah_evidence_level >= 2` 或有动脉瘤诊断/处置证据者，重复主 log-PCA K=3 流程。当前脚本输出 `phenotype_strict_aneurysm_subgroup_*` 系列表，用于回应“ICD 定义的 non-traumatic SAH 队列病因过杂”的审稿质疑。
...
- 对 8 个核心变量做 complete-case K=3 聚类，输出 `phenotype_complete_case_sensitivity` 和 `phenotype_complete_case_centers_zscore`。该分析不改变变量集合，仅排除任一核心变量缺失的样本，用于检验中位数填补是否驱动主分型。
- 去除 `hb_min_48h_all` 后用其余 7 个核心变量重新做 K-means K=3，输出 `phenotype_hb_free_sensitivity` 和 `phenotype_hb_free_centers_zscore`。这是贫血相关结果的关键反证分析：如果去除 Hb 后死亡率梯度仍存在、贫血率仍随表型加重而升高，说明贫血不是完全由“把 Hb 放进聚类后再报告贫血”造成的循环发现；如果梯度明显消失，主文必须降低贫血结论强度。
- 去除 `inr_max_48h` 后用其余 7 个核心变量重新做 K-means K=3，输出 `phenotype_inr_free_sensitivity` 和 `phenotype_inr_free_centers_zscore`。该分析只用于回应“INR 是否拖累模型几何表现”的质疑；如果去 INR 后 silhouette 没有改善或凝血特征解释变弱，应继续保留 INR 作为主聚类凝血功能维度。
- 原始 8 维 Z-score K-means K=3：当前脚本输出 `phenotype_raw_kmeans_assignments_sensitivity`、`phenotype_raw_kmeans_centers_zscore_sensitivity`、`phenotype_raw_kmeans_outcome_summary_sensitivity` 和 `phenotype_raw_kmeans_bootstrap_stability_sensitivity`。该方案用于说明未经 log/PCA 变换时死亡率梯度方向是否一致，不作为主结果。
- log-PCA 主分析审计副本：当前脚本同时输出 `phenotype_log_pca_kmeans_sensitivity`、`phenotype_log_pca_kmeans_centers_zscore`、`phenotype_log_pca_kmeans_loadings` 和 `phenotype_log_pca_kmeans_bootstrap_stability`，用于回应极端值拖尾、多重共线性和变换后 assignment 稳定性。该方案已升级为主几何空间，但必须报告前 3 个 PC 的实际解释方差，不能预设其达到 85%。
- LPA/GMM model-based clustering：当前脚本输出 `phenotype_lpa_fit_indices`、`phenotype_lpa_assignments_sensitivity`、`phenotype_lpa_outcome_summary_sensitivity`、`phenotype_lpa_centers_zscore_sensitivity` 和 `phenotype_lpa_severity_validation_sensitivity`。这项分析用于回应“为什么不用 LPA/model-based clustering”的方法学质疑。若 LPA/GMM 倾向 K=2/K=4、entropy 较低、最小类过小或与主 K-means ARI 低，应明确保留为补充材料，并说明偏态、边界值和离散评分使中心型 log-PCA K-means 更稳健、更便于临床解释。
- GCS 敏感性：GCS motor 主方案、加入 total GCS、total GCS only、GCS grade 替代 motor。
- 仅使用 PaO2/FiO2，限有 ABG 且可匹配 FiO2 的子集。

建议做：

- ePVS/troponin 候选增强聚类：只有在 `phenotype_candidate_feature_audit` 显示覆盖率高、单位一致、分布合理时才作为敏感性聚类；否则仅作描述变量。当前脚本输出 `phenotype_epvs_sensitivity_summary`，比较主 8 变量与加入 `epvs_mean_48h` 两种方案；ePVS 与 Hb/Hct 强相关，因此只能作为敏感性证据，不能作为主聚类独立维度。
...
- 在 MIMIC-IV 3.1 的成年 non-traumatic SAH ICU 患者中，0-48 小时常规低缺失生理指标经 log-PCA K-means 可识别 3 个早期表型。
- 优化后的 log-PCA K=3 更适合作为主文结果：样本量更均衡、稳定性更好，且能形成低风险、神经/循环受损和贫血-凝血-肾功能异常的多器官高危三个临床可解释模式。
- K=4 不应被丢弃，而应作为高分辨率探索：其小型极高危表型主要来自 K=3 重症组内部，提示真实世界中存在一个死亡率很高的极重症尾部。
- 早期贫血、INR 升高和低血小板与高危表型共同出现，并与死亡风险分层相关；这应解释为多模态生理表型的伴随特征和预后异质性提示，而不是贫血、INR 或输血获益的因果证据。当前 phenotype 内贫血/非贫血死亡率差异很小，交互项不显著，因此不能声称贫血在某一表型内具有独立或异质性预后效应。更稳妥的结论是：贫血是高危生理表型的一部分或下游特征，而非当前数据支持的独立死亡驱动因素。
...
## 12.3 后续升级路线：从静态极值到动态表型

当前 0-48 小时极值方案的优点是低缺失、可复现和临床可解释，但它只能描述早期窗口内的“最差状态”，不能捕捉生理恶化、恢复、波动和跨窗口转化。下一阶段应把动态表型作为独立工作包，而不是只在 Discussion 中作为一句 future work。

### 12.3.1 轨迹特征优先级

先构建不依赖新深度学习框架的轨迹摘要：

- 时间分段：0-6h、6-24h、24-48h、48-72h；若数据支持，再扩展到 ICU day 3-5 和 day 6-10。
- 变量：沿用主 8 变量，并优先加入测量密度较高的 HR/SBP/MAP/SpO2；实验室变量用实际测量点和分段 last/first/delta。
- 每个变量输出：first、last、min/max、time-weighted mean、slope、delta、variability、measurement_count、missing flag。
- 主问题：哪些 P1 患者在 24-72h 转为 P3-like physiology，哪些 P2 患者恢复到 P1-like physiology；转化矩阵和转化后死亡率应作为核心输出。

### 12.3.2 动态方法分层

建议按复杂度递进，而不是直接跳到黑箱模型：

1. 轨迹摘要 + PCA/K-means 或 GMM：最容易解释，也最容易与当前静态 K=3 对照。
2. Functional data analysis：适合把 MAP、SpO2、shock index 等高频变量表示为曲线后聚类。
3. Latent class mixed models：适合少量关键变量的可解释轨迹类别。
4. LSTM/Transformer：适合预测恶化或死亡，但需要单独处理不规则采样、缺失、mask、measurement intensity bias 和外部验证。若使用，主文应把它定位为动态表征/预测，不直接解释为生物亚型证据。

### 12.3.3 DCI 窗口

迟发性脑缺血通常在 SAH 后约 4-10 天达到高风险期。后续动态分析应把 0-48h 早期表型与 day 4-10 生理/治疗轨迹连接起来：

- 早期 P1/P2/P3 -> day 4-10 轨迹类别。
- 重点查看 P1 中延迟恶化和 P2/P3 中恢复轨迹。
- 若没有影像或临床确认 DCI，任何 vasospasm/DCI proxy 都必须标为 proxy，不得写成已确认 DCI 结局。

### 12.3.4 高级聚类和半监督表型

当前 LPA/GMM 已作为敏感性分析存在。后续可扩展：

- Gaussian Mixture Model/LPA：在 log-PCA 或轨迹低维空间中运行，报告 BIC/AIC/entropy/最小簇比例/ARI。
- HDBSCAN：用于非球形簇和噪声点，必须报告 noise fraction 和参数敏感性。
- Deep Embedded Clustering：只作为探索性；没有稳定性和外部验证时不能替代主 K-means。
- Semi-supervised phenotyping：如果用死亡、SOFA/APACHE、DCI proxy 参与表示学习，必须明确这是 outcome-informed risk strata，不再是纯无监督 discovery phenotype。

### 12.3.5 输血问题必须单独做因果设计

当前数据只能说明 RBC 输血率低、输血集中于高危表型、普通调整模型不能证明疗效。不能把“RBC 系数不显著”解释为“输血无效”。如果要研究输血：

- 先写 target trial emulation 方案，定义 time zero、eligible state、treatment strategy、grace period、follow-up、outcome 和 estimand。
- 使用 time-varying exposure 或 marginal structural model，避免 immortal-time bias。
- 倾向性评分匹配/加权必须报告 balance、overlap 和 positivity。
- 工具变量分析只有在存在可信工具变量时才做；弱工具或不满足排除限制的工具变量会比普通回归更误导。
- phenotype x transfusion 只能作为探索性 effect modification，不能给临床输血建议。
...
- `07_dynamic_trajectory_phenotypes.py`
  - 0-6h、6-24h、24-48h、48-72h 分段轨迹摘要，动态转化矩阵，轨迹聚类和 DCI 窗口扩展。
- `08_transfusion_target_trial.py`
  - 仅在正式做输血因果推断时创建；实现 target trial emulation、time-varying treatment/marginal structural model、PSM/IPTW 和必要诊断。
