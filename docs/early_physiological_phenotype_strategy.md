...
K-means 对缺失和插补敏感。如果缺失过多，聚类结果会反映“数据缺失模式”而不是生理表型。7 个核心变量最多缺失 2 个，可以在保留样本量的同时降低插补主导聚类的风险；Hb、total GCS 和 GCS grade 不计入主聚类完整度，仅用于暴露定义、描述和敏感性比较。
...
- Hb：最低值，用于贫血暴露定义和敏感性分析，不作为主聚类输入。
...
建议采用 7 个常规可提取、低缺失指标，对应文档中的 7 个生理维度。主聚类去掉 Hb 和 total GCS，仅保留 GCS motor 作为神经功能输入；加入最高 sodium 作为早期电解质/渗透压应激维度；同时彻底剔除高缺失的乳酸、PaO2/FiO2 和合成氧合比值；不构造“终末器官低灌注复合评分”，避免引入未经验证的自创评分工具。
...
| 电解质/渗透压应激 | `sodium_max_48h` | 0-48h 最高钠                                                   | 优先 `mimiciv_3_1_derived.chemistry.sodium`；用于捕捉早期高钠/渗透压相关应激                         |
...
- `hb_min_48h_all`、`hb_min_48h_pre_transfusion`：保留为贫血暴露定义、描述和输血敏感性分析变量，不作为主聚类变量，避免贫血暴露与表型输入重复。
...
- 使用 cluster center 的方向解释：例如低 GCS motor、低 MAP、高 shock index、低 SpO2、高 creatinine、高 sodium、低 platelet；Hb、total GCS 和 GCS grade 用于描述和敏感性验证。
...
- Table 1 至少包含年龄、性别、种族、入院类型、保险、aneurysm evidence level、动脉瘤诊断/处置证据、早期贫血、Hb、输血、ICU/住院时长、total GCS、GCS motor、GCS grade、ePVS、troponin、SAPS III/SOFA 和 7 个主聚类变量。Hb、total GCS 和 GCS grade 可作为描述或补充材料变量，不作为主聚类输入。
...
- ePVS/troponin 候选增强聚类：只有在 `phenotype_candidate_feature_audit` 显示覆盖率高、单位一致、分布合理时才作为敏感性聚类；否则仅作描述变量。当前脚本输出 `phenotype_epvs_sensitivity_summary`，比较主 7 变量与加入 `epvs_mean_48h` 两种方案；ePVS 与 Hb/Hct 强相关，因此只能作为敏感性证据，不能作为主聚类独立维度。
