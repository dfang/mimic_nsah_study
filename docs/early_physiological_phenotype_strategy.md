...
- `mimiciv_3_1_derived.gcs`：total GCS、GCS grade、GCS motor。
...
- Hb：最低值。
- Total GCS：最低 total GCS 用作主聚类神经功能变量；GCS grade 同步报告并可作为替代敏感性变量；GCS motor 同步保留，用于敏感性分析或预测性能比较。
- MAP：最低值。
- Shock index：最大值。
...
建议采用 8 个常规可提取、低缺失指标，对应文档中的 6 个生理维度。主聚类彻底剔除高缺失的乳酸、PaO2/FiO2 和合成氧合比值；不构造“终末器官低灌注复合评分”，避免引入未经验证的自创评分工具。
...
| 神经功能  | `gcs_min_48h`         | 0-48h 最低 total GCS                                           | 优先 `mimiciv_3_1_derived.gcs.gcs`                                                                   |
| 神经功能  | `gcs_grade_min_48h`   | 根据最低 total GCS 分级：轻度 `13-15`、中度 `9-12`、重度 `3-8` | 由 `gcs_min_48h` 派生；编码建议 1=轻度，2=中度，3=重度                                               |
...
- `gcs_motor_min_48h`：GCS motor 保留为敏感性分析和预测性能比较变量，不作为主聚类神经变量。
...
| Hb          | `3-25 g/dL`                                                                   |
| Total GCS   | `3-15`                                                                        |
| GCS grade   | `1-3`，其中 1=`13-15`，2=`9-12`，3=`3-8`                                      |
| GCS motor   | `1-6`，仅用于敏感性分析/预测比较                                              |
| MAP         | `30-200 mmHg`                                                                 |
| HR          | `30-220 bpm`                                                                  |
| SBP         | `50-260 mmHg`                                                                 |
...
- GCS 冗余敏感性：主聚类当前同时包含 `gcs_min_48h` 和由其派生的 `gcs_grade_min_48h`，两者信息重复。必须输出 `phenotype_gcs_sensitivity_summary`，比较双 GCS、仅 total GCS、仅 GCS grade 三套变量下 K=3 的样本量、死亡率、silhouette 和相对主方案的 ARI。若去掉任一 GCS 指标后主要表型结构仍相似，说明结果不是由重复编码强行驱动；若变化很大，主文应改用 total GCS only，GCS grade 只作描述。
- 对 24h 窗口重复聚类。
- 排除所有输血患者后重复聚类。
...
- 使用 cluster center 的方向解释：例如低 total GCS / 高 GCS grade、低 MAP、高 shock index、低 SpO2、低 Hb、高 creatinine、低 platelet。
...
- Table 1 至少包含年龄、性别、种族、入院类型、保险、aneurysm evidence level、动脉瘤诊断/处置证据、早期贫血、输血、ICU/住院时长、total GCS、GCS grade、ePVS、troponin、SAPS III/SOFA 和 8 个主聚类变量。GCS motor 可作为敏感性或补充材料变量，不作为主表神经功能代表。
...
- GCS 冗余敏感性：双 GCS、total GCS only、GCS grade only。
