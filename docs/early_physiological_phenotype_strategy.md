...
- Hb：最低值。
- Total GCS：最低 total GCS 用作主聚类神经功能变量；GCS motor 作为第二个主聚类神经功能变量；GCS grade 同步报告并可作为替代敏感性变量。
- MAP：最低值。
- Shock index：最大值。
...
- GCS 敏感性：主聚类当前同时包含 `gcs_min_48h` 和 `gcs_motor_min_48h`。必须输出 `phenotype_gcs_sensitivity_summary`，比较 total GCS + motor、仅 total GCS、仅 motor、以及用 GCS grade 替代 motor 四套变量下 K=3 的样本量、死亡率、silhouette 和相对主方案的 ARI。若去掉或替换任一 GCS 指标后主要表型结构仍相似，说明结果不是由单一 GCS 编码强行驱动。
- 对 24h 窗口重复聚类。
- 排除所有输血患者后重复聚类。
...
- 使用 cluster center 的方向解释：例如低 total GCS / 低 GCS motor、低 MAP、高 shock index、低 SpO2、低 Hb、高 creatinine、低 platelet。
...
- Table 1 至少包含年龄、性别、种族、入院类型、保险、aneurysm evidence level、动脉瘤诊断/处置证据、早期贫血、输血、ICU/住院时长、total GCS、GCS motor、GCS grade、ePVS、troponin、SAPS III/SOFA 和 8 个主聚类变量。GCS grade 可作为描述或补充材料变量，不作为主聚类神经功能代表。
...
- GCS 敏感性：total GCS + motor、total GCS only、GCS motor only、GCS grade 替代 motor。
