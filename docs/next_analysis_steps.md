...
当前主聚类使用 7 个低缺失变量：`hb_min_48h_all`、`gcs_motor_min_48h`、`map_min_48h`、`shock_index_max_48h`、`spo2_min_48h`、`creatinine_max_48h`、`platelet_min_48h`。`gcs_min_48h`、`gcs_grade_min_48h`、`lactate_max_48h`、`pao2_fio2_min_48h`、`spo2_fio2_min_48h` 和 `oxygenation_min_48h` 是描述或敏感性字段，不进入主聚类。
...
2. 7 个核心变量中有没有某个变量缺失率过高，尤其是 `spo2_min_48h` 和 `shock_index_max_48h`。
