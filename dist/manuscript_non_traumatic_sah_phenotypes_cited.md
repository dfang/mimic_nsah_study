...
P1 (n=694) was relatively preserved, P2 (n=384) had severe neurological dysfunction, and P3 (n=108) had neurological and multisystem dysfunction. Hospital mortality was 6.34%, 32.55%, and 61.11%. Adjusted odds ratios versus P1 were 7.59 (95% CI 5.07-11.36) for P2 and 21.21 (12.08-37.26) for P3. Mean patient-grouped bootstrap adjusted Rand index was 0.8554. Adjusted anemia odds ratios were 0.99 (0.68-1.44) with hemoglobin-containing phenotypes and 1.54 (1.06-2.22) after hemoglobin-free re-derivation. Frozen eICU assignment gave P1/P2/P3 counts of 539, 222, and 82; mortality was 5.4%/25.7%/42.7%.
...
Non-traumatic subarachnoid hemorrhage (NSAH) is a high-acuity neurological emergency with substantial mortality and long-term disability [@macdonald2017sah]. Clinical severity in SAH is commonly described using the Hunt-Hess and WFNS grades, together with the Glasgow Coma Scale (GCS) for level of consciousness [@hunt1968surgicalrisk; @teasdale1988wfns; @teasdale1974gcs]. These tools primarily capture neurological severity but do not incorporate the range of systemic measurements routinely collected in the ICU.
...
Unsupervised analyses have identified clinically distinct subgroups in sepsis and acute respiratory distress syndrome [@seymour2019sepsisphenotypes; @calfee2014ardssubphenotypes]. We applied this approach to early multimodal physiology in neurocritical care. We examined whether data-driven early physiological phenotypes in NSAH differed in observed in-hospital mortality and explored whether applying the frozen MIMIC-IV phenotype-assignment rule to another ICU database preserved the crude mortality ordering. We also examined whether the adjusted estimate of the association between early anemia and in-hospital mortality varied by phenotype specification.

## Methods

### Study design and data sources

We performed a retrospective cohort study using two de-identified, credentialed-access ICU databases: MIMIC-IV version 3.1, covering admissions during 2008-2022 [@johnson2023mimiciv; @johnson2024mimiciv31], and eICU-CRD version 2.0, covering 2014-2015 [@pollard2018eicu; @pollard2019eicu20]. Both resources are distributed through PhysioNet [@goldberger2000physionet]. MIMIC-IV was the derivation cohort; eICU-CRD was used for exploratory frozen transport of the MIMIC-derived phenotype-assignment rule. Database access followed the required training and data-use agreements. The protocol and statistical analysis plan were reconstructed after outcome access, so all analyses are exploratory. The final consent-waiver language remains subject to source-database governance and local ethics review. Reporting was structured with reference to STROBE and RECORD guidance [@vonelm2007strobe; @benchimol2015record], with separate checklist mappings.
...
The exploratory eICU assessment followed a frozen-transport design. MIMIC-derived imputation medians, transformations, scaling parameters, PCA projection, and phenotype centroids were exported to a versioned, access-controlled transform bundle before eICU evaluation. The evaluation program loaded that bundle and applied it without fitting, tuning, or recalibration on eICU data. The derived-sensitive bundle is not distributed publicly, but its SHA-256 hash, source commit, run identifier, and BigQuery job provenance were recorded to support authorized audit and exact replay of this execution. Because unsupervised clusters are data-dependent, interpretation focused on mortality and severity ordering rather than replication of patient-level cluster membership. Assigned groups were evaluated against mortality and eICU-provided severity variables not used for assignment. De novo eICU clustering was only a structural sensitivity analysis.
...
The three retained principal components explained 56.41% of total variance. The K=3 solution identified three clinically interpretable phenotypes (Fig. 1; ESM Fig. 6). P1 included 694 stays (58.5%) and had mild neurological and systemic impairment. P2 included 384 stays (32.4%) and had severe neurological impairment with relatively preserved systemic physiology. P3 included 108 stays (9.1%) and combined severe neurological impairment with hypotension, elevated shock index, hypoxemia, renal dysfunction, coagulopathy, thrombocytopenia, and lower hemoglobin. A 200-iteration, patient-grouped full-pipeline bootstrap supported good overall partition stability (mean adjusted Rand index, 0.8554).

![Phenotype heatmap](figures/fig2_primary_log_pca_heatmap.png)
**Fig. 1.** Early physiological profiles of the three MIMIC-derived phenotypes. The heatmap shows standardized cluster centers; raw medians and interquartile ranges are reported in ESM Table 8.
...
The eICU transport cohort included 843 patients from 66 hospitals. Frozen assignment placed 539 patients in P1, 222 in P2, and 82 in P3. Observed hospital mortality was 5.4%, 25.7%, and 42.7%, respectively. Before feature-completeness eligibility, INR was missing in 446 of 903 patients (49.4%); 390 of 843 transported patients (46.3%) required a MIMIC-derived median. ICU mortality, early anemia, and red blood cell transfusion followed the same crude order.

The assigned groups also had ordered eICU severity measures that were not used for assignment. Median APACHE scores were 36, 57, and 79 across P1, P2, and P3; Spearman rho was 0.480. Acute Physiology Score and predicted hospital mortality showed the same direction.

Hospital-clustered 95% intervals preserved separation (P2-P1 risk difference, 20.3 percentage points [15.1-25.8]; P3-P1, 37.3 [27.0-47.4]), and mortality ordering remained in all 66 leave-one-hospital-out analyses (ESM Table 10A).

Independent de novo K-means clustering in eICU recovered ordered mortality differences, but patient-level agreement with transported labels was negligible (adjusted Rand index -0.0017). The external results show a crude mortality ordering under frozen assignment, not replication of cluster boundaries across databases.

### Robustness

Sensitivity analyses retained the crude mortality order. The mean adjusted Rand index from 200 patient-grouped, full-pipeline bootstrap resamples was 0.8554, supporting overall partition stability; cluster-specific Jaccard values, individual assignment uncertainty, and multiple-seed stability were not assessed. INR-free eICU assignment yielded mortality of 4.6%, 24.4%, and 38.7% across P1 to P3. Hemoglobin-free clustering retained phenotype-mortality separation, but the anemia estimate changed from 0.99 to 1.54; the anemia result was not robust to phenotype specification.

## Discussion

Eight routine ICU measurements separated eligible stays into three early-course NSAH groups. Observed mortality increased from P1 to P3 in MIMIC, and frozen assignment produced the same crude order in eICU. P2 and P3 both had severe neurological impairment, whereas P3 also had more extensive multisystem physiological abnormalities.

Hunt-Hess and WFNS are SAH clinical grading systems, whereas GCS describes level of consciousness [@hunt1968surgicalrisk; @teasdale1988wfns; @teasdale1974gcs]. APACHE and SOFA summarize general illness severity and organ dysfunction [@knaus1985apacheii; @vincent1996sofa]. The present groups combine neurological and systemic measurements. Whether they are useful for cohort enrichment or trial stratification remains unknown.
...
**Code availability:** Public analysis code documents the transform contract. Exact MIMIC-derived transform parameters are treated as a derived-sensitive artifact and are not included in this public package; the private bundle hash and execution provenance are recorded so an authorized user can audit or replay the frozen evaluation in a controlled environment. Repository URL and institutional release approval must be completed before submission.
...
**Reporting guideline:** STROBE and RECORD checklist mappings [@vonelm2007strobe; @benchimol2015record] will be submitted as Electronic Supplementary Material.
