...
We conducted a retrospective two-database cohort study of adults with NSAH and an ICU stay >=24 h, using MIMIC-IV 3.1 (2008-2022) for phenotype derivation and eICU-CRD 2.0 (2014-2015) for external validation. Eight variables measured during the first 48 h represented neurological, circulatory, respiratory, renal, hematologic, and coagulation domains. After median imputation, skewed variables were log1p transformed, all variables were standardized, and K-means clustering was performed in a three-component principal component space. K=3 was selected using cluster metrics, minimum cluster size, bootstrap stability, and clinical interpretability. Associations with in-hospital mortality were evaluated using multivariable logistic regression. eICU validation used fixed MIMIC preprocessing parameters, loadings, and centroids.
...
Non-traumatic subarachnoid hemorrhage (NSAH) is a high-acuity neurological emergency with substantial mortality and long-term disability [@macdonald2017sah]. Early assessment remains challenging because patients with comparable neurological injury can follow markedly different clinical trajectories. Current risk stratification draws on neurological examination, radiographic findings, and clinical grading systems, including Hunt-Hess, WFNS, and the Glasgow Coma Scale (GCS) [@hunt1968surgicalrisk; @teasdale1988wfns; @teasdale1974gcs]. These measures remain central to assessment but do not incorporate the range of systemic physiology routinely measured in the ICU.

Studies of aneurysmal SAH describe systemic inflammation that may contribute to intracranial and extracranial injury [@chai2023inflammation] and frequent anemia during acute care [@rosenberg2013anemia]. Hemodynamic instability, impaired oxygenation, renal dysfunction, hematologic abnormalities, and coagulation disturbances may accompany the primary neurological insult. The extent and combination of these abnormalities can vary, motivating the study of early neuro-systemic response patterns rather than isolated measurements. APACHE and SOFA summarize general illness severity and organ dysfunction, respectively, but were not developed to identify NSAH-specific physiological patterns [@knaus1985apacheii; @vincent1996sofa].

Unsupervised analyses have identified clinically distinct subgroups in sepsis and acute respiratory distress syndrome [@seymour2019sepsisphenotypes; @calfee2014ardssubphenotypes]. Principal component analysis combined with K-means clustering provides a framework for identifying latent patterns in routinely collected data without imposing predefined clinical categories. The clinical relevance of such patterns, however, depends on transparent preprocessing, stability assessment, cautious interpretation, and evaluation in independent populations.

The relationship between early multidomain physiology and outcomes in NSAH remains incompletely characterized, and it is uncertain whether a phenotype classifier derived in one ICU database can preserve clinically relevant ordering in another. We therefore aimed to derive early physiological phenotypes among critically ill adults with NSAH, evaluate their associations with mortality and early anemia, and test fixed transport to an independent external ICU cohort. We hypothesized that the phenotypes would differ in observed mortality, that early anemia would add limited information after phenotype adjustment, and that the transported classifier would preserve mortality ordering in eICU.
...
Eight routinely available variables were selected a priori to represent early neurological and systemic physiology: minimum GCS motor score, minimum mean arterial pressure, maximum shock index, minimum oxygen saturation, maximum creatinine, maximum international normalized ratio, minimum hemoglobin, and minimum platelet count. Measurements were summarized during the first 48 h after ICU admission to characterize early physiology, while recognizing that treatment-related changes could occur within this window. Values outside clinically plausible ranges were removed. Missing values were imputed with derivation-cohort medians. Creatinine and international normalized ratio were log1p transformed because their distributions were right skewed. All variables were standardized with derivation-cohort means and standard deviations.

### Phenotype derivation and validation

Principal component analysis was applied to the standardized eight-variable matrix to reduce redundancy among correlated measurements and provide a lower-dimensional representation for clustering. Three components were retained and explained 56.41% of the variance. K-means clustering was performed in the three-component space using `random_state = 42` and `n_init = 100`. K=3 was selected from cluster metrics, minimum cluster size, bootstrap stability, and clinical interpretability. For presentation, phenotype labels were assigned post hoc using a direction-weighted physiological severity score calculated from standardized cluster centers. Mortality was used neither for clustering nor for label ordering.
...
Mortality increased across phenotypes (Fig. 2). In-hospital mortality was 6.34% in P1, 32.55% in P2, and 61.11% in P3 (p < 0.001). ICU mortality followed the same order: 3.60%, 26.56%, and 50.93%, respectively. In unadjusted Cox analysis, hazard ratios for hospital death were 4.20 (95% CI 2.97-5.94) for P2 and 7.94 (95% CI 5.38-11.70) for P3 compared with P1.
...
Using eight routine ICU measurements, we identified three early neuro-systemic physiological phenotypes among critically ill adults with NSAH. The phenotypes had distinct physiological profiles and progressively higher observed mortality. P2 and P3 both had severe neurological impairment, whereas P3 also had multisystem abnormalities and substantially higher mortality. A classifier derived in MIMIC-IV preserved this mortality ordering when transported to the independent, multicenter eICU database.

The contrast between P2 and P3 illustrates the main clinical finding: patients with similarly severe impairment in the neurological clustering variable can have markedly different systemic physiological profiles. P3 combined neurological impairment with hypotension, elevated shock index, hypoxemia, renal dysfunction, coagulopathy, thrombocytopenia, and lower hemoglobin. This pattern is consistent with a broader neuro-systemic severity axis, but the observational analysis cannot determine whether the systemic abnormalities caused the higher mortality.

Hunt-Hess, WFNS, and GCS emphasize neurological status [@hunt1968surgicalrisk; @teasdale1988wfns; @teasdale1974gcs], while APACHE and SOFA summarize general critical illness severity and organ dysfunction [@knaus1985apacheii; @vincent1996sofa]. The present phenotypes combine neurological and systemic domains and may complement these established assessments by describing different patterns of early physiology. We did not formally test incremental predictive performance over neurological or general severity scores; improved prognostication therefore cannot be inferred. Use of these phenotypes for cohort enrichment, trial stratification, or clinical decision-making requires prospective evaluation.

Early anemia was common in the high-risk phenotypes but was not independently associated with mortality after phenotype adjustment. Because hemoglobin contributed to phenotype derivation, its distribution across the primary phenotypes is partly structural and should not be treated as an independent discovery. Hemoglobin-free clustering preserved phenotype-mortality separation, suggesting that the overall gradient was not driven solely by hemoglobin. In this cohort, anemia therefore appears to be a marker of broader physiological derangement rather than an isolated prognostic factor. In the SAHARA trial, a liberal transfusion strategy did not significantly reduce unfavorable neurological outcome at 12 months compared with a restrictive strategy [@english2025sahara]. Our study does not estimate transfusion effects and should not guide transfusion thresholds. That question requires a causal analysis with an explicit treatment estimand or a randomized trial.

The external analysis supports transportability of the fixed classifier's mortality and severity ordering, not exact replication of cluster boundaries. De novo eICU clustering also recovered ordered mortality differences but did not reproduce the transported labels at the patient level. Differences in population characteristics, clinical practice, measurement, missingness, or documentation could explain this discordance, although we did not test those mechanisms. Preserved ordering in an independent multicenter cohort suggests that the observed associations were not restricted to the derivation population, but it does not establish universal generalizability. The fixed classifier warrants prospective evaluation, and these data do not show that fixed transport is superior to repeated de novo clustering.

This study has several strengths. The analysis used eight routinely available variables spanning neurological, circulatory, respiratory, renal, hematologic, and coagulation domains while retaining clinical interpretability. Prespecified preprocessing and fixed transport avoided refitting the primary phenotype model in eICU. Alternative observation windows, removal of selected variables, an alternative cluster solution, complete-case analysis, and bootstrap resampling examined robustness from complementary perspectives.
...
Unsupervised clusters are statistical patterns in the observed variables and should not be interpreted as fixed biological subtypes or mechanisms. Phenotype boundaries may vary across populations, as underscored by the low patient-level agreement between transported and de novo eICU labels. The transported P3 group was relatively small, so its outcome estimates are less precise. MIMIC-IV is a US single-center resource, and eICU is a US multicenter resource [@johnson2023mimiciv; @pollard2018eicu]. Prospective validation is needed in contemporary cohorts from other health systems, ideally with longitudinal measurements and biological markers.

## Conclusions

Early physiological measurements collected during routine ICU care identified three neuro-systemic phenotypes among critically ill adults with NSAH that had graded observed mortality. The MIMIC-derived classifier preserved this ordering in eICU and aligned with eICU-provided severity variables that were not used for phenotype assignment. Early anemia appeared to mark broader physiological derangement rather than remain independently associated with mortality after phenotype adjustment. Multidomain physiological assessment may complement neurological severity evaluation by characterizing heterogeneous systemic responses, and these phenotypes may help define strata for future studies of monitoring, prognostic communication, or targeted interventions, pending prospective validation.
