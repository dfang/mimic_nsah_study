...
**Status / 状态:** Static citation validation passed; actual citeproc rendering was not run. A multidisciplinary manuscript review was completed on 2026-07-16 and returned **REVISE**; the package is therefore **not frozen or submission-ready**. / 静态引文验证已通过，但未执行实际 citeproc 渲染；2026-07-16 完成多学科稿件审查，结论为 **REVISE**，因此本稿**未冻结，也未达到投稿就绪状态**。
...
- The verified citation-aware text was promoted to the canonical English and Chinese manuscripts; synchronized `_cited.md` copies are retained for traceability. / 经核验的引文版已提升为中英文规范稿；同步的 `_cited.md` 副本用于溯源。
...
- Both manuscripts use the same 18 unique keys in the same 18 citation clusters. All cited keys resolve in the bibliography, key map, evidence table, and verification table. / 中英文稿使用相同的 18 个唯一键及顺序一致的 18 个引文簇；所有已引键均可在书目、键映射、证据表和核验表中解析。
- Four eligible records are intentionally uncited: / 4 条合格记录有意未引用：
...
- Unresolved citation keys: **none**; all 18 cited keys resolve statically. / 未解析引文键：**无**；18 个已引键均通过静态解析。
...
- Static validation passed for key resolution and unused records; 18 synchronized keys and 18 synchronized clusters; 22 evidence rows × 31 columns and 35 verification rows × 17 columns; BibTeX entry types, balanced braces, authors, DOI/PMID identities and uniqueness; CSL XML and checksum; YAML metadata; and absence of hand-numbered references. / 静态验证通过项目包括：键解析及未用记录、18 个同步键与 18 个同步引文簇、22×31 证据表与 35×17 核验表、BibTeX 类型/花括号/作者/DOI/PMID 身份与唯一性、CSL XML 与校验和、YAML 及无手工编号参考文献。

## Canonical manuscript promotion / 规范稿提升

- The citation-aware English and Chinese manuscripts now occupy the canonical filenames required by this repository. / 引文版中英文稿现已使用仓库规定的规范文件名。
- `_cited.md` copies are synchronized traceability copies, not a competing manuscript version. / `_cited.md` 文件为同步溯源副本，不是并行竞争版本。
- Scientific estimates were retained during citation and language editing; prediction-model text was removed from the submission manuscript after methods review because it was not central and had unresolved cross-validation leakage risk. / 引文及语言编辑保留研究估计值；方法学审查发现交叉验证泄漏风险后，因预测模型并非主问题而将相关文字移出投稿稿件。

## Required next gate / 后续必需门槛

Install or provide a verified Pandoc/citeproc environment and render both manuscripts; inspect the generated references and links; complete ethics/consent wording; perform full-text checks where claims depend on details beyond abstracts; resolve the blocking and major findings in `manuscript_review_report.md`; then repeat manuscript and reproducibility review. Until those gates pass, this package must not be called frozen or submission-ready. / 后续须在经验证的 Pandoc/citeproc 环境中渲染中英文稿并检查参考文献与链接，补全伦理/知情同意表述，对依赖摘要之外细节的主张完成人工全文核验，解决 `manuscript_review_report.md` 中的阻断及重大问题，再重复稿件与可复现性审查。在这些门槛通过前，不得称本引文包已冻结或可投稿。
