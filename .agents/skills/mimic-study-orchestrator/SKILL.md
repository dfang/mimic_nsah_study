...
8. **Independent analysis review** — use `mimic-review` with `mode: gate` and `gate: analysis_review` on the frozen protocol, cohort, code, QC, analysis, and validation artifacts. Create an immutable input snapshot with a `review_run_id`; a builder must not silently self-approve a blocking issue.
9. **Manuscript** — use `mimic-clinical-manuscript`; derive claims and numbers only from frozen, reviewed results.
10. **Independent manuscript review** — use `mimic-review` with `mode: gate` and `gate: manuscript_review`, including `journal-reviewer`, on the complete manuscript, tables, figures, supplement, and reporting-checklist mapping. Create a new immutable input snapshot and `review_run_id` for every corrected version. Return blocking or major findings to the author and re-review the corrected version before freezing this gate.
...
12. **Submission** — use `mimic-submission-packager`, then `mimic-manuscript-compiler` when a rendered PDF is required; verify live journal instructions before packaging and visually inspect the PDF. In the submission manifest, persist both upstream gate records: the independent manuscript-review verdict artifact path and SHA-256, and the reproducibility-release manifest path and SHA-256, together with each gate's verdict and open `requires_redesign`, `blocks_ready`, blocking, and major finding counts.
