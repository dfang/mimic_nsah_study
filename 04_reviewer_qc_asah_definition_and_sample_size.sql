...
--
-- 运行前提：
-- 已经运行 01_create_phenotype_cohort.sql，并生成 asah_study 下的中间表。
--
-- 输出表：
...
CREATE OR REPLACE TABLE `mimic-study-498508.asah_study.reviewer_qc_attrition_by_rule` AS
WITH base AS (
    SELECT
        s.subject_id,
        s.hadm_id,
    ...
    LEFT JOIN `mimic-study-498508.asah_study.physiology_features_48h` f
...
SELECT *
FROM `mimic-study-498508.asah_study.reviewer_qc_attrition_by_rule`
ORDER BY step;
...
CREATE OR REPLACE TABLE `mimic-study-498508.asah_study.reviewer_qc_asah_definition_levels` AS
WITH source AS (
    SELECT
        s.subject_id,
        s.hadm_id,
...
SELECT *
FROM `mimic-study-498508.asah_study.reviewer_qc_asah_definition_levels`;
...
CREATE OR REPLACE TABLE `mimic-study-498508.asah_study.reviewer_qc_feature_availability_by_definition` AS
WITH labeled AS (
    SELECT
        CASE
...
SELECT
    ...
    COUNTIF(gcs_motor_min_48h IS NOT NULL) AS gcs_motor_sensitivity_nonmissing,
...
SELECT *
FROM `mimic-study-498508.asah_study.reviewer_qc_feature_availability_by_definition`
ORDER BY definition_level;
...
CREATE OR REPLACE TABLE `mimic-study-498508.asah_study.reviewer_qc_procedure_code_audit` AS
SELECT
    pi.icd_version,
    pi.icd_code,
    dp.long_title,
...
LEFT JOIN `mimic-study-498508.asah_study.dx_sah_flags` s
...
SELECT *
FROM `mimic-study-498508.asah_study.reviewer_qc_procedure_code_audit`
ORDER BY rows_with_nontrauma_sah_dx DESC, procedure_rows DESC;
...
INNER JOIN `physionet-data.mimiciv_3_1_hosp.d_icd_procedures` dp
    ON pi.icd_code = dp.icd_code
   AND pi.icd_version = dp.icd_version
INNER JOIN `mimic-study-498508.asah_study.source_sah_admissions` s
    ON pi.subject_id = s.subject_id
   AND pi.hadm_id = s.hadm_id
WHERE (
        LOWER(dp.long_title) LIKE '%clip%'
...
CREATE OR REPLACE TABLE `mimic-study-498508.asah_study.reviewer_qc_included_diagnosis_titles` AS
SELECT
    di.icd_version,
    di.icd_code,
    dd.long_title,
...
SELECT *
FROM `mimic-study-498508.asah_study.reviewer_qc_included_diagnosis_titles`
ORDER BY admissions DESC, diagnosis_rows DESC;

-- 目的：列出可能有创伤性或非动脉瘤性提示的主分析病例，供人工复核。
-- 原因：如果该表非空，需要逐例判断是否应排除或作为敏感性分析。
CREATE OR REPLACE TABLE `mimic-study-498508.asah_study.reviewer_qc_possible_traumatic_or_nonaneurysmal` AS
SELECT DISTINCT
    f.subject_id,
    f.hadm_id,
    f.stay_id,
    ...
    di.icd_code,
    dd.long_title
FROM `mimic-study-498508.asah_study.physiology_features_48h` f
...
SELECT *
FROM `mimic-study-498508.asah_study.reviewer_qc_possible_traumatic_or_nonaneurysmal`
...
SELECT
    (SELECT admissions FROM `mimic-study-498508.asah_study.reviewer_qc_attrition_by_rule` WHERE step = '01_all_sah_icd_admissions') AS broad_sah_admissions,
    (SELECT admissions FROM `mimic-study-498508.asah_study.reviewer_qc_attrition_by_rule` WHERE step = '03_strict_asah_evidence_level_ge_2') AS strict_asah_admissions,
    (SELECT admissions FROM `mimic-study-498508.asah_study.reviewer_qc_attrition_by_rule` WHERE step = '05_has_first_icu_stay') AS strict_asah_with_icu,
    (SELECT admissions FROM `mimic-study-498508.asah_study.reviewer_qc_attrition_by_rule` WHERE step = '07_core_missing_le_2') AS core_features_complete_enough,
    (SELECT admissions FROM `mimic-study-498508.asah_study.reviewer_qc_attrition_by_rule` WHERE step = '08_primary_analysis_no_massive_transfusion') AS primary_analysis_n,
    (SELECT COUNT(*) FROM `mimic-study-498508.asah_study.reviewer_qc_possible_traumatic_or_nonaneurysmal`) AS possible_trauma_or_nonaneurysmal_records_to_review;
